 ...
 */
inline void removeDuplicates(vector<CompleteEdge>& input, vector<CompleteEdge>& output) {
 ...
 */
inline NodeID readGraphFromStream(istream &in, const bool adaptWeight, const bool addIsolatedNode,
 ...
 *       separately.
 */
inline datastr::graph::UpdateableGraph* importGraphListOfEdgesUpdateable(istream &inGraph, const bool adaptWeight, const bool addIsolatedNode, const string filenameHwyNodes) {
    vector<CompleteEdge> edgeList;
    ...
    cout << edgeList.size() << " - " << edgeListWithoutDuplicates.size() << endl;
