...
public:
    ...
    * @return a reference to the given path object that accepts the result
     */
    PathCH& pathTo(PathCH& path, NodeID t, int searchID = 0, bool forward = true, bool expand = false) {
        path.clear();
     ...
     * reads the path that should be expanded from a PathCH object instead of the Dijkstra search tree.
    ...
    void expandPath(const PathCH& path, PathCH& pathExpanded) 
    {
     ...
     */
    void expandEdge(PathCH& path, NodeID& t, NodeID& parent, EdgeID& e, bool& processSecondHalf, const bool forward)
    {
...
private:
    ...
    */
    void clearNodeVector(vector<NodeID>& nodeVector) {
    ...
    void clear(int searchID) {
        ...
        clearNodeVector(_settledNodes[searchID]);
...
typedef processing::DijkstraCH<datastr::graph::SearchGraph, NormalPQueue, 2, true, true, processing::MTMM_FW, false, false> DijkstraCHManyToManyFW;
typedef processing::DijkstraCH<datastr::graph::UpdateableGraph, DynQueryPQueue, 2, true, true, processing::MTMM_FW, false, false >DijkstraCHManyToManyFW2;

/** Used for the many-to-many backward search. */
typedef processing::DijkstraCH<datastr::graph::SearchGraph, NormalPQueue, 2, true, true, processing::MTMM_BW, false, false> DijkstraCHManyToManyBW;
typedef processing::DijkstraCH<datastr::graph::UpdateableGraph, DynQueryPQueue, 2, true, true, processing::MTMM_BW, false, false> DijkstraCHManyToManyBW2;
