...
class PathCH
{
    ...
    friend ostream& operator<<( ostream& os, const PathCH& p ) {
        if (p._notConnected) {
...
public:
    ...
    bool operator== (const PathCH& p) const {
     ...
     *              occurence of a difference and ends just after the last
     */
    void diff(const PathCH& p, PathCH& diff1, PathCH& diff2) const {
        diff1.clear();
        diff2.clear();
...
private:
    ...
    void subpath(PathCH& p, NodeID begin, NodeID end) const {
...
class Paths : public vector<PathCH>
{
public:
    /**
    ...
    void diff(ostream& os, const Paths& p, bool verbose = false) const {
        ...
        PathCH diff1, diff2; // store the differences of one comparison of two paths
        vector< pair<PathCH, PathCH> > diffs; // stores the differences of all comparisons,
        ...
        for (NodeID i = 0; i < size(); i++) {
            ...
            if ((! diff1.empty()) || (! diff2.empty())) { // if both paths aren't identical
                ...
                pair<PathCH, PathCH> newDiff( diff1, diff2 );
                bool unique = true;
